...
class RestoreFileResultInfo(object):
    ...
    This message captures result of restore of individual file or directory as
    initiated from magneto. Note, it is expected that the agents go through the
    "estimation" phase first for the entire batch of restore requests and then
    start copying. These state transitions are reflected in the "status" field
    here.


    Attributes:

        copy_stats (RestoreFileCopyStats): This captures the copy stats for
            this file and is present when the status is kInProgress.
        destination_dir (string): This is set to the destination directory
            where the file/directory was copied.
        error (ErrorProto): This is set if there is any error during the
            restore.
        restored_file_info (RestoredFileInfo): Information of the file being
            restored. Note, the agent does not need to populate this.
