...
class ProtectionRunParameters(object):

    """Implementation of the 'Protection Run Parameters.' model.

    Specify the parameters to run a protection job.

    Attributes:
        copy_run_targets (list of CopyTaskJobRunParameters): Optional
            parameter to be set if you want specific replication or archival
            associated with the policy to run.
        run_now_parameters (list of RunNowParameters): Optional parameters of
            a Run Now operation.
        run_type (RunType2Enum): Specifies the type of backup. If not
            specified, 'kRegular' is assumed. 'kRegular' indicates a
    ...
    def __init__(self,
                 ...
                 run_type=None,
                 source_ids=None):
        """Constructor for the ProtectionRunParameters class"""
