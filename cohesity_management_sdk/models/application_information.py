...
class ApplicationInformation(object):
    ...
    """Implementation of the 'Application Information.' model.

    Specifies the application registered on the Cohesity Cluster.

    Attributes:
        application_tree_info (list of NodeInAProtectionSourcesTree):
            Application Server and the subtrees below them.  Specifies the
            application subtree used to store additional application level
            Objects. Different environments use the subtree to store
            application level information. For example for SQL Server, this
            subtree stores the SQL Server instances running on a VM or a
            Physical Server. overrideDescription: true
        environment (Environment4Enum): Specifies the environment type of the
            application such as 'kSQL', 'kExchange' registered on the
    ...
    def __init__(self,
                 ...
                 environment=None):
        """Constructor for the ApplicationInformation class"""
    ...
    def from_dictionary(cls,
        ...
        if dictionary.get('applicationTreeInfo') != None:
            ...
            for structure in dictionary.get('applicationTreeInfo'):
                application_tree_info.append(cohesity_management_sdk.models.node_in_a_protection_sources_tree.NodeInAProtectionSourcesTree.from_dictionary(structure))
