...
class EnvironmentTypeJobParameters(object):
    ...
    Attributes:
        ...
        aws_snapshot_parameters (AwsSnapshotManagerParameters): Specifies
            additional special parameters that are applicable only to snaphot
            manger jobs.
        exchange_parameters (ExchangeEnvJobParameters): Specifies additional
            special parameters that are applicable only to Types of 'kExchange'
            type.
        externally_triggered_job_parameters
            (ExternallyTriggeredEnvJobParameters): Specifies additional special
            parameters that are applicable only to externally triggered backup
            jobs of 'kView' type.
        hyperv_parameters (HypervEnvJobParameters): Specifies additional
            special parameters that are applicable only to Types of 'kHyperV'
            type.
        ...
        office_365_parameters (Office365EnvJobParameters): Specifies additional
            ...
            backup parameters.
        oracle_parameters (OracleEnvJobParameters): Specifies additional
            special parameters that are applicable only to Types of 'kOracle'
            type.
        physical_parameters (PhysicalEnvJobParameters): Specifies additional
            special parameters that are applicable only to Sources of
            'kPhysical' type in a kPhysical environment.
        pure_parameters (SanEnvJobParameters): Specifies additional special
            parameters that are applicable only to SAN Types.
        sql_parameters (SqlEnvJobParameters): Specifies additional special
            parameters that are applicable only to Types of 'kSQL' type.
        vmware_parameters (VmwareEnvJobParameters): Specifies additional
            special parameters that are applicable only to Types of 'kVMware'
            type.
    """
