...
class ADObjectRestoreParam(object):
    ...
    Attributes:
        ...
        credentials (Credentials): For restoring user type objects (user,
            inetOrgPerson or organizationalPerson LDAP classes) that is
            returned in search result with 'kRestorePasswordRequired' flag, an
            initial password is required. The password is UTF8 string encoded.
            ...
            restored from production AD recycle bin, password will not be
            changed and the restored user retains their original password. For
            non-user type objects, this password value is ignored. The username
            field is ignored in the credentials.
