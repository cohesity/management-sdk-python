...
class IsilonEnvParams_ZoneConfig(object):
    ...
    Attributes:
        ...
        dynamic_network_pool_config
            (IsilonEnvParams_ZoneConfig_NetworkPoolConfig): Dynamic network
            pool configuration for the access zone. Dynamic pool is used for
            stateless protocols, e.g. NFSv3.
        groupnet (string): Name of the zone's groupnet.
        smb_credentials (NasMountCredentials): SMB credentials for the access
            zone.
        static_network_pool_config
            (IsilonEnvParams_ZoneConfig_NetworkPoolConfig): Static network pool
            configuration for the access zone. Static pool is used for stateful
            protocols: e.g. SMB, NFSv4, and for stateless protocols e.g. NFSv3
            if dynamic pool below is not set.
    """
