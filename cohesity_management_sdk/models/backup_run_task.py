...
import cohesity_management_sdk.models.protection_job_run_statistics

class BackupRunTask(object):

    """Implementation of the 'Backup Run Task.' model.
    ...
    Attributes:
        environment (Environment6Enum): Specifies the environment type that
            the task is protecting. Supported environment types such as
        ...
        source_backup_status (list of SourceObjectBackupStatus): Array of
            Source Object Backup Status.  Specifies the status of backing up
            each source objects (such as VMs) associated with the job.
        stats (ProtectionJobRunStatistics): Specifies statistics about a
            Protection Job Run. This contains the Job Run level statistics.
        status (Status2Enum): Specifies the status of Backup task such as
            'kRunning', 'kSuccess', 'kFailure' etc. 'kAccepted' indicates the
            task is queued to run but not yet running. 'kRunning' indicates
            the task is running. 'kCanceling' indicates a request to cancel
            the task has occurred but the task is not yet canceled.
    ...
    def __init__(self,
        ...
        """Constructor for the BackupRunTask class"""
    ...
    def from_dictionary(cls,
        ...
        if dictionary.get('sourceBackupStatus') != None:
            ...
            for structure in dictionary.get('sourceBackupStatus'):
                source_backup_status.append(cohesity_management_sdk.models.source_object_backup_status.SourceObjectBackupStatus.from_dictionary(structure))
        stats = cohesity_management_sdk.models.protection_job_run_statistics.ProtectionJobRunStatistics.from_dictionary(dictionary.get('stats')) if dictionary.get('stats') else None
