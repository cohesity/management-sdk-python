...
import cohesity_management_sdk.models.deprecated_in_501
import cohesity_management_sdk.models.snapshot_target_policy_proto

class DEPRECATEDIn50(object):

    """Implementation of the 'DEPRECATED in 5.0!' model.

    A message that specifies the policies to use for a job.

    Attributes:
        backup_policy (DEPRECATEDIn501): If a backup does not get a chance to
            when it's due (either due to the system being busy or a conflict
            with another instance of the same job), the backup will still be
            run when the conflicts go away. But, if there are multiple
    ...
    def __init__(self,
        ...
        """Constructor for the DEPRECATEDIn50 class"""
    ...
    def from_dictionary(cls,
        ...
        backup_policy = cohesity_management_sdk.models.deprecated_in_501.DEPRECATEDIn501.from_dictionary(dictionary.get('backupPolicy')) if dictionary.get('backupPolicy') else None
        snapshot_target_policy_vec = None
