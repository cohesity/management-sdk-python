...
class InputSpec_FileTimeFilter(object):
    ...
    File time filter to filter files based on their last change time. All files
    whose change time is in the range [change_time_range_start_secs,
    change_time_range_end_secs) will be processed. Both values are time
    duration in seconds w.r.t. to current time and not absolute points in time.


    Attributes:

        change_time_range_end_secs (long|int): End of file's change time range.
    ...
    def from_dictionary(cls,
        ...
        change_time_range_end_secs = dictionary.get('changeTimeRangeEndSecs')
        change_time_range_start_secs = dictionary.get('changeTimeRangeStartSecs')

        # Return an object of this model
        return cls(
            change_time_range_end_secs,
