...
class SqlAagHostAndDatabases(object):
    ...
    Specifies AAGs and databases information on an SQL server. If AAGs exist on
    the server, specifies information about the AAG and databases in the group
    for each AAG found on the server.
    ...
    Attributes:
        ...
        application_node (ProtectionSourceNode): Specifies the subtree used to
            store application-level Objects. Different environments use the
            subtree to store application-level information. For example for SQL
            Server, this subtree stores the SQL Server instances running on a
            VM or Physical hosts.
        ...
        error_message (string): Specifies an error message when the host is not
            registered as an SQL host.
