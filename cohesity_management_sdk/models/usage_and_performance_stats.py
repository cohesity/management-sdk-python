...
class UsageAndPerformanceStats(object):
    ...
    Attributes:
        ...
        system_capacity_bytes (long|int): Provides the total available capacity
            as computed by the Linux 'statfs' command.
        system_usage_bytes (long|int): Provides the usage of bytes, as computed
            by the Linux 'statfs' command, after the size of the data is
            reduced by change-block tracking, compression and deduplication.
        total_physical_raw_usage_bytes (long|int): Provides the usage of bytes,
            as computed by the Cohesity Cluster, before the size of the data is
            reduced by change-block tracking, compression and deduplication.
        ...
        write_ios (long|int): Provides the number of Write IOs that occurred in
            the last 30 seconds.
        write_latency_msecs (float): Provides the Write latency in milliseconds
            for the Write IOs that occurred during the last 30 seconds.
    """
