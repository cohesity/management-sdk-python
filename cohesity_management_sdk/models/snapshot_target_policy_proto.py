...
class SnapshotTargetPolicyProto(object):
    ...
    Attributes:
        ...
        granularity_bucket (GranularityBucket): Message that specifies the
            ...
            job's runs.
        id (string): This id identifies an entry in a global policy. For local
            policies, this id is used for merging the contents with a
            corresponding global policy.  This id is guaranteed to be unique
            within a ProtectionPolicyProto. A SnapshotTargetPolicyProto in a
            local linked policy that corresponds to one in its parent parent
            policy will have the same id.  This id is generated by Helios and
            Magneto treats it as an opaque identifier.
