...
class SqlEnvJobParameters(object):
    ...
    Attributes:
        ...
        aag_preference (AagPreferenceEnum): Specifies the preference for
            ...
            implies no preference of about whether backups are performed on the
            primary replica or on a secondary replica. If no secondary replica
        ...
        backup_system_databases (bool): If true, system databases are backed
            up. If this is set to false, system databases are not backed up. If
            this field is not specified, default value is true.
        backup_type (BackupTypeEnum): Specifies the type of the 'kFull' backup
            job. Specifies whether it is Volume level backup or individual
            files level backup. kSqlVSSVolume implies volume based VSS full
            backup. kSqlVSSFile implies file based VSS full backup.
        backup_volumes_only (bool): If set to true, only the volumes associated
            with databases should be backed up. The user cannot select
            additional volumes at host level for backup.  If set to false, all
            the volumes on the host machine will be backed up. In this case,
            the user can further select the exact set of volumes using host
            level params.  Note that the volumes associated with selected
            databases will always be included in the backup.
        incremental_snapshot_upon_restart (bool): If true, the backup of type
            kSqlVssVolume will be incremental after restart
        is_copy_only_full (bool): If true, the backup is a full backup with the
        ...
        with_clause (string): With clause is used for setting clauese in native
            sql backup.
    """
