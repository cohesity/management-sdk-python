...
class RestoreWrapperProto(object):
    ...
    of the restore job/task and the list of all destroy tasks (only when the
    record is for a restore task of type clone) associated with it. If this
    message is delta record, it will contain the state mutation for one of
    individual restore job, restore task and individual destroy task.


    Attributes:

        destroy_cloned_task_state_vec (list of DestroyClonedTaskStateProto):
            For a restore task of type 'Clone', this field contains the info of
            the destroy task(s).
        owner_restore_wrapper_proto (RestoreWrapperProto): Note that this field
            is only populated for Iris in response to 'GetRestoreTasksArg' RPC.
        perform_refresh_task_state_vec (list of PerformRestoreTaskStateProto):
            Contains information of the refresh tasks for a clone
        perform_restore_job_state (PerformRestoreJobStateProto): Contains the
            information about a restore job. When the info of the restore job
            is send to the Iris, we will transform this proto to the above
            'perform_restore_task_state' so that Iris has to deal with only one
            kind of restore proto.
        perform_restore_task_state (PerformRestoreTaskStateProto): Contains
            information about a restore task.
        restore_sub_task_wrapper_proto_vec (list of object): If this restore
            ...
            only populated for Iris in response to 'GetRestoreTasksArg' RPC. It
            is not persisted in Magneto's WAL.  List of environments that use
