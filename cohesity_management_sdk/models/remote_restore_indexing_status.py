...
class RemoteRestoreIndexingStatus(object):
    ...
    Attributes:
        ...
        indexing_task_end_time_usecs (long|int): Specifies when the indexing
            ...
            microseconds). This field is not set if the indexing task is still
            in progress.
        ...
        indexing_task_status (IndexingTaskStatusEnum): Specifies the status of
            the indexing Job/task. 'kJobRunning' indicates that the Job/task is
        ...
        latest_expiry_time_usecs (long|int): For all the Snapshots retrieved by
            this Job, specifies the latest time when a Snapshot expires.
        progress_monitor_task (string): Specifies the path to progress monitor
            task to track the progress of building the index.
        start_time_usecs (long|int): Specifies the start time of the time range
            that is being indexed. The indexing task is creating an index of
            the Job Runs that occurred between this startTimeUsecs and the
            endTimeUsecs. This field is recorded as a Unix epoch Timestamp (in
            microseconds).
    """
