# coding: utf-8

"""
    Cohesity REST API

    This API provides operations for interfacing with the Cohesity Cluster. NOTE: To view the documentation on the responses, click 'Model' next to 'Example Value' and keep clicking to expand the hierarchy.

    OpenAPI spec version: 1.0
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.
"""

from pprint import pformat
from six import iteritems
import re


class UpdateClusterParams(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self, bonding_mode=None, cluster_audit_log_config=None, dns_server_ips=None, domain_names=None, enable_active_monitoring=None, enable_upgrade_pkg_polling=None, encryption_key_rotation_period_secs=None, filer_audit_log_config=None, gateway=None, is_documentation_local=None, language_locale=None, mtu=None, name=None, ntp_settings=None, reverse_tunnel_enabled=None, smb_ad_disabled=None, syslog_servers=None, timezone=None, turbo_mode=None):
        """
        UpdateClusterParams - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'bonding_mode': 'str',
            'cluster_audit_log_config': 'ClusterAuditLogConfiguration',
            'dns_server_ips': 'list[str]',
            'domain_names': 'list[str]',
            'enable_active_monitoring': 'bool',
            'enable_upgrade_pkg_polling': 'bool',
            'encryption_key_rotation_period_secs': 'int',
            'filer_audit_log_config': 'FilerAuditLogConfiguration',
            'gateway': 'str',
            'is_documentation_local': 'bool',
            'language_locale': 'str',
            'mtu': 'int',
            'name': 'str',
            'ntp_settings': 'NtpSettingsConfig',
            'reverse_tunnel_enabled': 'bool',
            'smb_ad_disabled': 'bool',
            'syslog_servers': 'list[SyslogServer]',
            'timezone': 'str',
            'turbo_mode': 'bool'
        }

        self.attribute_map = {
            'bonding_mode': 'bondingMode',
            'cluster_audit_log_config': 'clusterAuditLogConfig',
            'dns_server_ips': 'dnsServerIps',
            'domain_names': 'domainNames',
            'enable_active_monitoring': 'enableActiveMonitoring',
            'enable_upgrade_pkg_polling': 'enableUpgradePkgPolling',
            'encryption_key_rotation_period_secs': 'encryptionKeyRotationPeriodSecs',
            'filer_audit_log_config': 'filerAuditLogConfig',
            'gateway': 'gateway',
            'is_documentation_local': 'isDocumentationLocal',
            'language_locale': 'languageLocale',
            'mtu': 'mtu',
            'name': 'name',
            'ntp_settings': 'ntpSettings',
            'reverse_tunnel_enabled': 'reverseTunnelEnabled',
            'smb_ad_disabled': 'smbAdDisabled',
            'syslog_servers': 'syslogServers',
            'timezone': 'timezone',
            'turbo_mode': 'turboMode'
        }

        self._bonding_mode = bonding_mode
        self._cluster_audit_log_config = cluster_audit_log_config
        self._dns_server_ips = dns_server_ips
        self._domain_names = domain_names
        self._enable_active_monitoring = enable_active_monitoring
        self._enable_upgrade_pkg_polling = enable_upgrade_pkg_polling
        self._encryption_key_rotation_period_secs = encryption_key_rotation_period_secs
        self._filer_audit_log_config = filer_audit_log_config
        self._gateway = gateway
        self._is_documentation_local = is_documentation_local
        self._language_locale = language_locale
        self._mtu = mtu
        self._name = name
        self._ntp_settings = ntp_settings
        self._reverse_tunnel_enabled = reverse_tunnel_enabled
        self._smb_ad_disabled = smb_ad_disabled
        self._syslog_servers = syslog_servers
        self._timezone = timezone
        self._turbo_mode = turbo_mode

    @property
    def bonding_mode(self):
        """
        Gets the bonding_mode of this UpdateClusterParams.
        Specifies the bonding mode to use when bonding NICs to this Cluster. 'KActiveBackup' indicates an Active-backup policy bonding mode. 'K802_3ad' indicates an EEE 802.3ad Dynamic link aggregation bonding mode. 'KBalanceAlb' indicates a Adaptive load balancing bonding mode.

        :return: The bonding_mode of this UpdateClusterParams.
        :rtype: str
        """
        return self._bonding_mode

    @bonding_mode.setter
    def bonding_mode(self, bonding_mode):
        """
        Sets the bonding_mode of this UpdateClusterParams.
        Specifies the bonding mode to use when bonding NICs to this Cluster. 'KActiveBackup' indicates an Active-backup policy bonding mode. 'K802_3ad' indicates an EEE 802.3ad Dynamic link aggregation bonding mode. 'KBalanceAlb' indicates a Adaptive load balancing bonding mode.

        :param bonding_mode: The bonding_mode of this UpdateClusterParams.
        :type: str
        """
        allowed_values = ["ActiveBackup", "802_3ad", "BalanceAlb"]
        if bonding_mode not in allowed_values:
            raise ValueError(
                "Invalid value for `bonding_mode` ({0}), must be one of {1}"
                .format(bonding_mode, allowed_values)
            )

        self._bonding_mode = bonding_mode

    @property
    def cluster_audit_log_config(self):
        """
        Gets the cluster_audit_log_config of this UpdateClusterParams.
        Cluster Audit Log Configuration.

        :return: The cluster_audit_log_config of this UpdateClusterParams.
        :rtype: ClusterAuditLogConfiguration
        """
        return self._cluster_audit_log_config

    @cluster_audit_log_config.setter
    def cluster_audit_log_config(self, cluster_audit_log_config):
        """
        Sets the cluster_audit_log_config of this UpdateClusterParams.
        Cluster Audit Log Configuration.

        :param cluster_audit_log_config: The cluster_audit_log_config of this UpdateClusterParams.
        :type: ClusterAuditLogConfiguration
        """

        self._cluster_audit_log_config = cluster_audit_log_config

    @property
    def dns_server_ips(self):
        """
        Gets the dns_server_ips of this UpdateClusterParams.
        Specifies the IP addresses of the DNS Servers used by the Cohesity Cluster.

        :return: The dns_server_ips of this UpdateClusterParams.
        :rtype: list[str]
        """
        return self._dns_server_ips

    @dns_server_ips.setter
    def dns_server_ips(self, dns_server_ips):
        """
        Sets the dns_server_ips of this UpdateClusterParams.
        Specifies the IP addresses of the DNS Servers used by the Cohesity Cluster.

        :param dns_server_ips: The dns_server_ips of this UpdateClusterParams.
        :type: list[str]
        """

        self._dns_server_ips = dns_server_ips

    @property
    def domain_names(self):
        """
        Gets the domain_names of this UpdateClusterParams.
        The first domain name specified in the array is the fully qualified domain name assigned to the Cohesity Cluster. Any additional domain names specified are used for the domain search list for hostname look-up.

        :return: The domain_names of this UpdateClusterParams.
        :rtype: list[str]
        """
        return self._domain_names

    @domain_names.setter
    def domain_names(self, domain_names):
        """
        Sets the domain_names of this UpdateClusterParams.
        The first domain name specified in the array is the fully qualified domain name assigned to the Cohesity Cluster. Any additional domain names specified are used for the domain search list for hostname look-up.

        :param domain_names: The domain_names of this UpdateClusterParams.
        :type: list[str]
        """

        self._domain_names = domain_names

    @property
    def enable_active_monitoring(self):
        """
        Gets the enable_active_monitoring of this UpdateClusterParams.
        Specifies if Cohesity can receive monitoring information from the Cohesity Cluster. If 'true', remote monitoring of the Cohesity Cluster is allowed.

        :return: The enable_active_monitoring of this UpdateClusterParams.
        :rtype: bool
        """
        return self._enable_active_monitoring

    @enable_active_monitoring.setter
    def enable_active_monitoring(self, enable_active_monitoring):
        """
        Sets the enable_active_monitoring of this UpdateClusterParams.
        Specifies if Cohesity can receive monitoring information from the Cohesity Cluster. If 'true', remote monitoring of the Cohesity Cluster is allowed.

        :param enable_active_monitoring: The enable_active_monitoring of this UpdateClusterParams.
        :type: bool
        """

        self._enable_active_monitoring = enable_active_monitoring

    @property
    def enable_upgrade_pkg_polling(self):
        """
        Gets the enable_upgrade_pkg_polling of this UpdateClusterParams.
        If 'true', Cohesity's upgrade server is polled for new releases.

        :return: The enable_upgrade_pkg_polling of this UpdateClusterParams.
        :rtype: bool
        """
        return self._enable_upgrade_pkg_polling

    @enable_upgrade_pkg_polling.setter
    def enable_upgrade_pkg_polling(self, enable_upgrade_pkg_polling):
        """
        Sets the enable_upgrade_pkg_polling of this UpdateClusterParams.
        If 'true', Cohesity's upgrade server is polled for new releases.

        :param enable_upgrade_pkg_polling: The enable_upgrade_pkg_polling of this UpdateClusterParams.
        :type: bool
        """

        self._enable_upgrade_pkg_polling = enable_upgrade_pkg_polling

    @property
    def encryption_key_rotation_period_secs(self):
        """
        Gets the encryption_key_rotation_period_secs of this UpdateClusterParams.
        Specifies the period of time (in seconds) when encryption keys are rotated. By default, the encryption keys are rotated every 77760000 seconds (30 days).

        :return: The encryption_key_rotation_period_secs of this UpdateClusterParams.
        :rtype: int
        """
        return self._encryption_key_rotation_period_secs

    @encryption_key_rotation_period_secs.setter
    def encryption_key_rotation_period_secs(self, encryption_key_rotation_period_secs):
        """
        Sets the encryption_key_rotation_period_secs of this UpdateClusterParams.
        Specifies the period of time (in seconds) when encryption keys are rotated. By default, the encryption keys are rotated every 77760000 seconds (30 days).

        :param encryption_key_rotation_period_secs: The encryption_key_rotation_period_secs of this UpdateClusterParams.
        :type: int
        """

        self._encryption_key_rotation_period_secs = encryption_key_rotation_period_secs

    @property
    def filer_audit_log_config(self):
        """
        Gets the filer_audit_log_config of this UpdateClusterParams.
        Filer Audit Log Configuration.

        :return: The filer_audit_log_config of this UpdateClusterParams.
        :rtype: FilerAuditLogConfiguration
        """
        return self._filer_audit_log_config

    @filer_audit_log_config.setter
    def filer_audit_log_config(self, filer_audit_log_config):
        """
        Sets the filer_audit_log_config of this UpdateClusterParams.
        Filer Audit Log Configuration.

        :param filer_audit_log_config: The filer_audit_log_config of this UpdateClusterParams.
        :type: FilerAuditLogConfiguration
        """

        self._filer_audit_log_config = filer_audit_log_config

    @property
    def gateway(self):
        """
        Gets the gateway of this UpdateClusterParams.
        Specifies the gateway IP address.

        :return: The gateway of this UpdateClusterParams.
        :rtype: str
        """
        return self._gateway

    @gateway.setter
    def gateway(self, gateway):
        """
        Sets the gateway of this UpdateClusterParams.
        Specifies the gateway IP address.

        :param gateway: The gateway of this UpdateClusterParams.
        :type: str
        """

        self._gateway = gateway

    @property
    def is_documentation_local(self):
        """
        Gets the is_documentation_local of this UpdateClusterParams.
        Specifies what version of the documentation is used. If 'true', the version of documentation stored locally on the Cohesity Cluster is used. If 'false', the documentation stored on a Cohesity Web Server is used. The default is 'false'. Cohesity recommends accessing the Help from the Cohesity Web site which provides the newest and most complete version of Help.

        :return: The is_documentation_local of this UpdateClusterParams.
        :rtype: bool
        """
        return self._is_documentation_local

    @is_documentation_local.setter
    def is_documentation_local(self, is_documentation_local):
        """
        Sets the is_documentation_local of this UpdateClusterParams.
        Specifies what version of the documentation is used. If 'true', the version of documentation stored locally on the Cohesity Cluster is used. If 'false', the documentation stored on a Cohesity Web Server is used. The default is 'false'. Cohesity recommends accessing the Help from the Cohesity Web site which provides the newest and most complete version of Help.

        :param is_documentation_local: The is_documentation_local of this UpdateClusterParams.
        :type: bool
        """

        self._is_documentation_local = is_documentation_local

    @property
    def language_locale(self):
        """
        Gets the language_locale of this UpdateClusterParams.
        Specifies the language and locale for this Cohesity Cluster.

        :return: The language_locale of this UpdateClusterParams.
        :rtype: str
        """
        return self._language_locale

    @language_locale.setter
    def language_locale(self, language_locale):
        """
        Sets the language_locale of this UpdateClusterParams.
        Specifies the language and locale for this Cohesity Cluster.

        :param language_locale: The language_locale of this UpdateClusterParams.
        :type: str
        """

        self._language_locale = language_locale

    @property
    def mtu(self):
        """
        Gets the mtu of this UpdateClusterParams.
        Specifies the Maxium Transmission Unit (MTU) in bytes of the network.

        :return: The mtu of this UpdateClusterParams.
        :rtype: int
        """
        return self._mtu

    @mtu.setter
    def mtu(self, mtu):
        """
        Sets the mtu of this UpdateClusterParams.
        Specifies the Maxium Transmission Unit (MTU) in bytes of the network.

        :param mtu: The mtu of this UpdateClusterParams.
        :type: int
        """

        self._mtu = mtu

    @property
    def name(self):
        """
        Gets the name of this UpdateClusterParams.
        Specifies the name of the Cohesity Cluster.

        :return: The name of this UpdateClusterParams.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """
        Sets the name of this UpdateClusterParams.
        Specifies the name of the Cohesity Cluster.

        :param name: The name of this UpdateClusterParams.
        :type: str
        """

        self._name = name

    @property
    def ntp_settings(self):
        """
        Gets the ntp_settings of this UpdateClusterParams.
        Specifies if the ntp/master slave scheme should be disabled for this cluster.

        :return: The ntp_settings of this UpdateClusterParams.
        :rtype: NtpSettingsConfig
        """
        return self._ntp_settings

    @ntp_settings.setter
    def ntp_settings(self, ntp_settings):
        """
        Sets the ntp_settings of this UpdateClusterParams.
        Specifies if the ntp/master slave scheme should be disabled for this cluster.

        :param ntp_settings: The ntp_settings of this UpdateClusterParams.
        :type: NtpSettingsConfig
        """

        self._ntp_settings = ntp_settings

    @property
    def reverse_tunnel_enabled(self):
        """
        Gets the reverse_tunnel_enabled of this UpdateClusterParams.
        If 'true', Cohesity's Remote Tunnel is enabled. Cohesity can access the Cluster and provide remote assistance via a Remote Tunnel.

        :return: The reverse_tunnel_enabled of this UpdateClusterParams.
        :rtype: bool
        """
        return self._reverse_tunnel_enabled

    @reverse_tunnel_enabled.setter
    def reverse_tunnel_enabled(self, reverse_tunnel_enabled):
        """
        Sets the reverse_tunnel_enabled of this UpdateClusterParams.
        If 'true', Cohesity's Remote Tunnel is enabled. Cohesity can access the Cluster and provide remote assistance via a Remote Tunnel.

        :param reverse_tunnel_enabled: The reverse_tunnel_enabled of this UpdateClusterParams.
        :type: bool
        """

        self._reverse_tunnel_enabled = reverse_tunnel_enabled

    @property
    def smb_ad_disabled(self):
        """
        Gets the smb_ad_disabled of this UpdateClusterParams.
        Specifies if Active Directory should be disabled for authentication of SMB shares. If 'true', Active Directory is disabled.

        :return: The smb_ad_disabled of this UpdateClusterParams.
        :rtype: bool
        """
        return self._smb_ad_disabled

    @smb_ad_disabled.setter
    def smb_ad_disabled(self, smb_ad_disabled):
        """
        Sets the smb_ad_disabled of this UpdateClusterParams.
        Specifies if Active Directory should be disabled for authentication of SMB shares. If 'true', Active Directory is disabled.

        :param smb_ad_disabled: The smb_ad_disabled of this UpdateClusterParams.
        :type: bool
        """

        self._smb_ad_disabled = smb_ad_disabled

    @property
    def syslog_servers(self):
        """
        Gets the syslog_servers of this UpdateClusterParams.
        Specifies a list of Syslog servers to send audit logs to.

        :return: The syslog_servers of this UpdateClusterParams.
        :rtype: list[SyslogServer]
        """
        return self._syslog_servers

    @syslog_servers.setter
    def syslog_servers(self, syslog_servers):
        """
        Sets the syslog_servers of this UpdateClusterParams.
        Specifies a list of Syslog servers to send audit logs to.

        :param syslog_servers: The syslog_servers of this UpdateClusterParams.
        :type: list[SyslogServer]
        """

        self._syslog_servers = syslog_servers

    @property
    def timezone(self):
        """
        Gets the timezone of this UpdateClusterParams.
        Specifies the timezone to use for showing time in emails, reports, filer audit logs, etc.

        :return: The timezone of this UpdateClusterParams.
        :rtype: str
        """
        return self._timezone

    @timezone.setter
    def timezone(self, timezone):
        """
        Sets the timezone of this UpdateClusterParams.
        Specifies the timezone to use for showing time in emails, reports, filer audit logs, etc.

        :param timezone: The timezone of this UpdateClusterParams.
        :type: str
        """

        self._timezone = timezone

    @property
    def turbo_mode(self):
        """
        Gets the turbo_mode of this UpdateClusterParams.
        Specifies if the cluster is in Turbo mode.

        :return: The turbo_mode of this UpdateClusterParams.
        :rtype: bool
        """
        return self._turbo_mode

    @turbo_mode.setter
    def turbo_mode(self, turbo_mode):
        """
        Sets the turbo_mode of this UpdateClusterParams.
        Specifies if the cluster is in Turbo mode.

        :param turbo_mode: The turbo_mode of this UpdateClusterParams.
        :type: bool
        """

        self._turbo_mode = turbo_mode

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other
