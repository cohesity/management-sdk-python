# coding: utf-8

"""
    Cohesity REST API

    This API provides operations for interfacing with the Cohesity Cluster. NOTE: To view the documentation on the responses, click 'Model' next to 'Example Value' and keep clicking to expand the hierarchy.

    OpenAPI spec version: 1.0
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.
"""

from pprint import pformat
from six import iteritems
import re


class UserQuotaAndUsage(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self, quota_policy=None, sid=None, unix_uid=None, usage_bytes=None):
        """
        UserQuotaAndUsage - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'quota_policy': 'QuotaPolicy',
            'sid': 'str',
            'unix_uid': 'int',
            'usage_bytes': 'int'
        }

        self.attribute_map = {
            'quota_policy': 'quotaPolicy',
            'sid': 'sid',
            'unix_uid': 'unixUid',
            'usage_bytes': 'usageBytes'
        }

        self._quota_policy = quota_policy
        self._sid = sid
        self._unix_uid = unix_uid
        self._usage_bytes = usage_bytes

    @property
    def quota_policy(self):
        """
        Gets the quota_policy of this UserQuotaAndUsage.
        User quota policy applied to this user.

        :return: The quota_policy of this UserQuotaAndUsage.
        :rtype: QuotaPolicy
        """
        return self._quota_policy

    @quota_policy.setter
    def quota_policy(self, quota_policy):
        """
        Sets the quota_policy of this UserQuotaAndUsage.
        User quota policy applied to this user.

        :param quota_policy: The quota_policy of this UserQuotaAndUsage.
        :type: QuotaPolicy
        """

        self._quota_policy = quota_policy

    @property
    def sid(self):
        """
        Gets the sid of this UserQuotaAndUsage.
        If interested in a user via smb_client, include SID. Otherwise, If valid unix-id to SID mappings are available (i.e., when mixed mode is enabled) the server will perform the necessary id mapping and return the correct usage irrespective of whether the unix id / SID is provided. The string is of following format - S-1-IdentifierAuthority-SubAuthority1-SubAuthority2-...-SubAuthorityn.

        :return: The sid of this UserQuotaAndUsage.
        :rtype: str
        """
        return self._sid

    @sid.setter
    def sid(self, sid):
        """
        Sets the sid of this UserQuotaAndUsage.
        If interested in a user via smb_client, include SID. Otherwise, If valid unix-id to SID mappings are available (i.e., when mixed mode is enabled) the server will perform the necessary id mapping and return the correct usage irrespective of whether the unix id / SID is provided. The string is of following format - S-1-IdentifierAuthority-SubAuthority1-SubAuthority2-...-SubAuthorityn.

        :param sid: The sid of this UserQuotaAndUsage.
        :type: str
        """

        self._sid = sid

    @property
    def unix_uid(self):
        """
        Gets the unix_uid of this UserQuotaAndUsage.
        If interested in a user via unix-identifier, include UnixUid. Otherwise, If valid unix-id to SID mappings are available (i.e., when mixed mode is enabled) the server will perform the necessary id mapping and return the correct usage irrespective of whether the unix id / SID is provided.

        :return: The unix_uid of this UserQuotaAndUsage.
        :rtype: int
        """
        return self._unix_uid

    @unix_uid.setter
    def unix_uid(self, unix_uid):
        """
        Sets the unix_uid of this UserQuotaAndUsage.
        If interested in a user via unix-identifier, include UnixUid. Otherwise, If valid unix-id to SID mappings are available (i.e., when mixed mode is enabled) the server will perform the necessary id mapping and return the correct usage irrespective of whether the unix id / SID is provided.

        :param unix_uid: The unix_uid of this UserQuotaAndUsage.
        :type: int
        """

        self._unix_uid = unix_uid

    @property
    def usage_bytes(self):
        """
        Gets the usage_bytes of this UserQuotaAndUsage.
        Current logical usage of user id in the input view.

        :return: The usage_bytes of this UserQuotaAndUsage.
        :rtype: int
        """
        return self._usage_bytes

    @usage_bytes.setter
    def usage_bytes(self, usage_bytes):
        """
        Sets the usage_bytes of this UserQuotaAndUsage.
        Current logical usage of user id in the input view.

        :param usage_bytes: The usage_bytes of this UserQuotaAndUsage.
        :type: int
        """

        self._usage_bytes = usage_bytes

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other
