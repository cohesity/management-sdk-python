...
def update_view_whitelist(args):
    ...
    try:
        cohesity_client.views.update_view_by_name(name=args.nfs_share_name, body=body)
    except APIException as ex:
        print("Failed to update %s , reason: %s" % (args.nfs_share_name, ex))
        exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--whitelist_ip", help="IP to whitelist Eg: 10.0.0.1", required=True
    )
    parser.add_argument(
        "--nfs_share_name", help="Name of Cohesity NFS/View", required=True
    )
    parser.add_argument(
        "--whitelist_netmask",
