...
def register_nas_source(nas_source):
    url = (
        "https://%s/irisservices/api/v1/backupsources"
        % cohesity_client.config.cluster_vip
    )
    cohesity_client.auth.authorize(cohesity_client.config)
    print(cohesity_client.config.auth_token)
    bearer_token = cohesity_client.config.auth_token.access_token
    data = {
        "entity": {
            "type": 11,
            "genericNasEntity": {"protocol": 1, "type": 1, "path": nas_source},
        },
        "entityInfo": {"endpoint": nas_source, "type": 11},
    }
    headers = {"Authorization": "Bearer %s" % bearer_token}
    resp = requests.post(url=url, data=json.dumps(data), headers=headers, verify=False)
    if not resp.ok:
        print(
            "Registration of %s unsuccessfull, skipping protection job "
            "creation." % nas_source
        )
...
def get_policy_id(policy_name):
    try:
        policy_resp = cohesity_client.protection_policies.get_protection_policies(
            names=policy_name
        )
...
def get_storage_domain_id(storage_domain):
    try:
        sd_resp = cohesity_client.view_boxes.get_view_boxes(names=storage_domain)
        return sd_resp[0].id
    except:
...
def create_nas_protection_job(nas_source, object_id):
    try:
        ...
        body.name = (
            parser.get("cohesity", "job_name_prefix") + nas_source.split(":/")[1]
        )
        body.policy_id = get_policy_id(parser.get("cohesity", "policy"))
        body.view_box_id = get_storage_domain_id(
            parser.get("cohesity", "storage_domain")
        )
        ...
        print(
            "Unable to create Protection Job for %s due to: %s"
            % (nas_source, ex)
        )
    ...
    try:
        ...
        if len(resp) == 0:  # This means protection run has not started.
            cohesity_client.protection_jobs.create_run_protection_job(
                id=job_id, body=RunProtectionJobParam()
            )
    except APIException as ex:
        print("Unable to start Protection Job for due to: %s" % ex)
...
def main():
    for _, nas_source in parser.items("nas_export_list"):
        ...
        pj_name = parser.get("cohesity", "job_name_prefix") + nas_source.split(":/")[1]
        print(
            "Protection Job %s created for the NAS target: %s "
