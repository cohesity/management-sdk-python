...
class ProtectionJobs(object):
    ...
    def name_to_job_id(self, job_name):
        ...
        """
        results = self.jobs_controller.get_protection_jobs(
    ...
    def run_job(self, job_name):
        ...
        jresp = self.run_controller.get_protection_runs(job_id=job_id, num_runs=1)
        if jresp == []:
            time.sleep(20)
            jresp = self.run_controller.get_protection_runs(job_id=job_id, num_runs=1)[
                0
            ...
            jresp = jresp[0]
        if (
            jresp.backup_run.status == StatusSourceBackupStatusEnum.KSUCCESS
            or jresp.backup_run.status == StatusSourceBackupStatusEnum.KACCEPTED
        ):
